...
export default class MeuProjeto extends Component {
...
Alert.alert(JSON.stringify(item))
    ...
    <Card style={styles.card}  >
        ...
        <View style={{flexDirection:'row', flex:1, alignItems: 'center', alignSelf:'flex-end' }}>
          ...
          <TouchableOpacity onPress={()=> this.props.navigation.navigate('AtividadesProjeto',{projeto: item, usuario_id: this.state.usuario_id})} style={{marginTop:15, height:45, width:150, padding:5, alignItems:'center', marginRight:10, backgroundColor:'#FD5979',borderColor:'#00BFFF', borderRadius:10}}>
            <Text style={{color:'#fff', margin:5, fontWeight:'bold'}} >Atividades</Text>
          </TouchableOpacity>
            )}
...
mudarTela = async (projeto) => {

const id =this.state.projeto[0].projeto_id  

Alert.alert(JSON.stringify(this.state.projeto[0]))

this.props.navigation.navigate('AtividadesProjeto',{projeto_id: id})
}
...
<Text style={{color:'#fff', fontWeight:'bold', fontSize:18, margin:10}} >Meus projetos</Text>
...
const url = 'http://projetofaculdade.herokuapp.com/projetoUsuario';
  fetch(url, {
      method: 'POST',
        headers: {
           'Accept': 'application/json', 
           'Content-Type': 'application/json',
           'Access-Control-Allow-Origin': 'http://projetofaculdade.herokuapp.com/projetoUsuario',
            'Access-Control-Allow-Credentials': true,
      },
      body: JSON.stringify({
